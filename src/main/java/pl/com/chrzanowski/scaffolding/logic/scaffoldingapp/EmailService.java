...
//    }
    ...
    public void sendPasswordResetMail(PasswordResetTokenData token) {
        ...
        sentEmailsService.create(new SentEmailData(null, token.getUser().getId(), title, content, locale.toLanguageTag()
    ...
    public void sendNewsletterMail(UserData user, NewsletterData newsletter) {
        ...
        sentEmailsService.create(new SentEmailData(null, user.getId(), title, content, locale.toLanguageTag(), MailEvent.NEWSLETTER.getCode()));
    }

    public void sendEmailConfirmationLink(UserData customer) {
