...
import pl.com.chrzanowski.scaffolding.api.PurchasedCoursesAdminRequestGetResponse;
...
import pl.com.chrzanowski.scaffolding.logic.courseplatform.notifications.NotificationsFromPanelService;
...
public class CoursePlatformEndpointAdmin {
    ...
    public void createCustomer(@RequestBody CustomerPostRequest request, HttpServletRequest httpServletRequest) {
        courseCustomersService.registerCustomer(new CustomerData(request.getLogin(), request.getPasswordHash(), request.getLanguage(), request.getRegulationAccepted(), request.getNewsletterAccepted(), request.getInvoiceFirstAndLastName(), request.getIsEnabled(), request.getIsEmailConfirmed(), request.getAuthorities(), httpServletRequest));
    }
    ...
    public List<StatisticNewCustomerData> newCustomers() {
        return statisticService.findNewCustomers();
    }
