...
import pl.com.chrzanowski.scaffolding.logic.adviser.*;
import pl.com.chrzanowski.scaffolding.logic.lifeadviser.LifeAdviserService;
...
public class LifeAdviserEndpoint {
    ...
    @PostMapping(path = "purchase-start", consumes = "application/json; charset=UTF-8", produces = "application/json; charset=UTF-8")
    public PurchasePostResponse purchaseStart(@RequestBody PurchasePostRequest req) {
        applicationsService.validateApplication(req.getApplicationId(), req.getSecret());
        Long id = lifeAdviserService.startPurchase(req.getDomainId(), req.getCategoryId(), req.getApplicationId(), req.getDomain(), req.getPeriod());
    ...
    public List<MyCategoriesGetResponse> myCategories(
        ...
        List<AdvicePurchasedCategoryData> list = adviceCategoriesService.findPurchasedCategories(
                new AdvicePurchasedCategoriesFilter(page, pageSize, domainId, appId, domain));
    ...
    private List<MyCategoriesGetResponse> prepareMyCategories(List<AdvicePurchasedCategoryData> list) {
        ...
        for (AdvicePurchasedCategoryData adviceCategoryData : list) {
            MyCategoriesGetResponse data = new MyCategoriesGetResponse(adviceCategoryData.getId(), adviceCategoryData.getName(), adviceCategoryData.getDescription(),
                    adviceCategoryData.getPrice().toString(), adviceCategoryData.getPeriod().name(), adviceCategoryData.getDateFrom().toString(),
                    adviceCategoryData.getDateTo().toString(), adviceCategoryData.getStatus().name());
            result.add(data);
        }
        return result;
    }
    ...
    private List<TriggeredAdviceGetResponse> prepareTriggeredAdvices(List<TriggeredAdviceData> list) {
        ...
        for (TriggeredAdviceData triggeredAdviceData : list) {
            TriggeredAdviceGetResponse data = new TriggeredAdviceGetResponse(triggeredAdviceData.getId(), triggeredAdviceData.getContent(), triggeredAdviceData.getScore(), triggeredAdviceData.getName());
            result.add(data);
        }
        return result;
    }
