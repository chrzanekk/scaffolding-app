...
import cleaning.toDelete.domain.courseplatform.AuthorData;
import cleaning.toDelete.domain.courseplatform.AuthorsFilter;
import cleaning.toDelete.domain.courseplatform.CustomerData;
import cleaning.toDelete.domain.courseplatform.CustomersFilter;
...
public class CourseCustomersService {
    ...
    public CourseCustomersService(CourseCustomersJdbcRepository courseCustomersJdbcRepository, EmailService emailService, CustomerAuthoritiesService customerAuthoritiesService, AuthorsService authorsService) {
    ...
    public List<CustomerData> findWithAuthorities(CustomersFilter filter) {
        List<CustomerData> customers = courseCustomersJdbcRepository.find(filter);
        List<CustomerData> customersWithRoles = new ArrayList<>();
        if (customers == null) {
            customersWithRoles = null;
        } else {
            for (CustomerData customer : customers) {
                customersWithRoles.add(
                        new CustomerData(customer, customerAuthoritiesService.getAuthoritiesForCustomer(customer)));
            }
        }
    ...
    private void createTeacherNewAccount(CustomerData teacherCreateRequest) {
        CustomerData customer = new CustomerData(teacherCreateRequest.getLogin(), teacherCreateRequest.getPasswordHash(),
                teacherCreateRequest.getLanguage(), teacherCreateRequest.getRegulationAccepted(),
                teacherCreateRequest.getNewsletterAccepted(), true, teacherCreateRequest.getRegistrationDatetime(),
                teacherCreateRequest.getRegistrationIp(), teacherCreateRequest.getRegistrationUserAgent(), false);
...
//            customerAuthoritiesService.validateAndCreateAuthorityForCustomer(customer, CustomerAuthority.TEACHER);
    ...
    private void createNewCustomerAccount(CustomerData customerCreateRequest) {
        CustomerData customer = new CustomerData(customerCreateRequest.getLogin(), customerCreateRequest.getPasswordHash(),
                customerCreateRequest.getLanguage(), customerCreateRequest.getRegulationAccepted(),
                customerCreateRequest.getNewsletterAccepted(), customerCreateRequest.getEnabled(), customerCreateRequest.getInvoiceFirstAndLastName(), customerCreateRequest.getRegistrationDatetime(),
                customerCreateRequest.getRegistrationIp(), customerCreateRequest.getRegistrationUserAgent(), customerCreateRequest.getEmailConfirmed());
        Long customerId = create(customer);
        CustomerData createdCustomer = find(new CustomersFilter(customerId)).get(0);
